...
const UserSchema = mongoose.Schema(
  {
    ...
    email: {
      ...
      minlength: [10],
      maxlength: [30],
      validate: [validation.pattern.email, "email format is not valid"],
    },
    password: { 
      type: String,
      required: true,
      ...
      validate: [validation.pattern.password, "first name must contain only letters"],
    },
    firstName: {
      type: String,
      required: true,
      minlength: [3],
      maxlength: [30],
      validate: [validation.pattern.name, "first name should only include a-z/A-Z characters"],
    },
    lastName: {
      type: String,
      required: true,
      minlength: [3],
      maxlength: [30],
      validate: [validation.pattern.name, "last name should only include a-z/A-Z characters"],
    },
