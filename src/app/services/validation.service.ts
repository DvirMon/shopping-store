...
export class ValidationService {
  ...
  public regex = {
    ...
    phone: /^\+?(972\-?)?0?(([23489]{1}\-?\d{7})|[5]{1}\d{1}\-?\d{7})$/,
  ...
  public validRegex(prop: string, value: string): boolean {
    return this.regex[prop].test(value);
  }
  ...
  public personalIdUniqueValidation(control: AbstractControl): Observable<ValidationErrors | null> {
    ...
    return control.valueChanges.pipe(
      ...
      take(1),
      switchMap(payload => {
        return this.http.post(`${environment.server}/api/valid/unique-personalId`, { personalId: payload }).pipe(
          map((error: boolean) => {
