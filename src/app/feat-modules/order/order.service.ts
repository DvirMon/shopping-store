...
import {
  OrderHistoryModel,
  OrderModel,
} from './components/order-form/order-model';
...
import {
  historySelector,
  yearsSelector,
} from 'src/app/utilities/ngrx/selectors';
import { OrderState } from 'src/app/utilities/ngrx/state/orders.state';
...
export class OrderService {
  ...
  public ordersHistory$: Observable<OrderHistoryModel[]> =
    this.store.select(historySelector);
  ...
  public orderEntries$: Observable<OrderHistoryModel[]> =
    this.ordersSearchEntries.asObservable();
  ...
  public getOrdersHistory(date: number) {
    this.http
      .get<OrderHistoryModel[]>(this.url + `/history/${this.user._id}/${date}`)
      .subscribe((history: OrderHistoryModel[]) => {
  ...
  public getOrdersYeras() {
    this.http
      .get<number[]>(this.url + `/years/${this.user._id}`)
  ...
  private searchOrders(query: string): Observable<OrderHistoryModel[]> {
    return this.http.get<OrderHistoryModel[]>(
      this.url + `/search/${this.user._id}/${query}`
    );
  }
  ...
  public createToken(
    ...
    stripeService.createToken(card.element, { name }).subscribe((result) => {
      if (result.token) {
        // Use the token
